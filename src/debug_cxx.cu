...
class Debug::Impl
{
    ...
    const int SZ_BRU = sizeof(_op_bru)/sizeof(int);
    ...
    const int SZ_ALU =	sizeof(_op_alu)/sizeof(int);

    vector<int>_op_jmp = {
        OP_JMP, OP_JMPIF, OP_JMPNOT, OP_ONERR
    };
    const int SZ_JMP = sizeof(_op_jmp)/sizeof(int);
    ...
    const int SZ_SYM = sizeof(_op_sym)/sizeof(int);

    vector<int>_op_exe = {
        OP_SEND, OP_SENDB, OP_CLASS, OP_MODULE, OP_METHOD
    };
    const int SZ_EXE = sizeof(_op_exe)/sizeof(int);
...
Debug::Debug(U32 flag) : _impl(new Impl(flag)) {}
Debug::~Debug() = default;

Debug *Debug::getInstance(U32 flag)
{
	static Debug *_self = NULL;

	if (!_self) {
		_self = new Debug(flag);
	}
	return _self;
}
