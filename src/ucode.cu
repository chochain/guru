/*! @file
  @brief
  GURU instruction unit - prefetch & microcode dispatcher

  <pre>
  Copyright (C) 2019 GreenII

  1. a list of opcode (microcode) executor, and
  2. the core opcode dispatcher
  </pre>
*/
#include <assert.h>

#include "alloc.h"
#include "static.h"
#include "symbol.h"
#include "global.h"
#include "ucode.h"
#include "object.h"
#include "ostore.h"
#include "class.h"
#include "state.h"

#if GURU_USE_STRING
#include "c_string.h"
#endif
#if GURU_USE_ARRAY
#include "c_range.h"
#include "c_array.h"
#include "c_hash.h"
#endif

#include "puts.h"

#define _LOCK		{ MUTEX_LOCK(_mutex_uc); }
#define _UNLOCK		{ MUTEX_FREE(_mutex_uc); }

__GURU__ U32 _mutex_uc;
//
// becareful with the following macros, because they release regs[ra] first
// so, make sure value is kept before the release
//
#define _RA(v)      	(ref_dec(&regs[ra]), regs[ra]=(v))
#define _RA_X(r)    	(ref_dec(&regs[ra]), regs[ra]=*(r), ref_inc(r))
#define _RA_T(t, e) 	(ref_dec(&regs[ra]), regs[ra].gt=(t), regs[ra].e)

#define _AR(r)          ((vm->ar.r))
#define _Ax				((vm->bytecode>>7) & 0x1ffffff)
#define _R0             (&(vm->state->regs[0]))
#define _R(r)			(&(vm->state->regs[_AR(r)]))
#define _RA2(v)			(ref_dec(_R(a)), *_R(a)=(v))
#define _RA_X2(r)    	(ref_dec(_R(a)), *_R(a)=*(r), ref_inc(r))
#define _RA_T2(t,e)     (_R(a)->gt=(t), _R(a)->e)

#define SKIP(x)	        { guru_na(x); return; }
#define QUIT(x)			{ vm->quit=1; guru_na(x); return; }
//================================================================
/*!@brief
  OP_NOP

  No operation
*/
__GURU__ void
uc_nop(guru_vm *vm)
{
}

//================================================================
/*!@brief
  OP_MOVE

  R(A) := R(B)
*/
__GURU__ void
uc_move(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
	U32 ra = GET_RA(code);
	U32 rb = GET_RB(code);
*/
    _RA_X2(_R(b)); 	                	// [ra] <= [rb]
}

//================================================================
/*!@brief
  OP_LOADL

  R(A) := Pool(Bx)
*/
__GURU__ void
uc_loadl(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
	U32 ra = GET_RA(code);
    U32 bx = GET_Bx(code);
*/
	U32  bx = _AR(bx);
    U32P p  = VM_VAR(vm, bx);
    guru_obj obj;

    if (*p & 1) {
    	obj.gt = GT_FLOAT;
    	obj.f  = *(GF *)p;
    }
    else {
    	obj.gt = GT_INT;
    	obj.i  = *p>>1;
    }
    _RA2(obj);
}

//================================================================
/*!@brief
  OP_LOADI

  R(A) := sBx
*/
__GURU__ void
uc_loadi(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra   = GET_RA(code);
    U32 sbx  = GETARG_sBx(code);

    _RA_T(GT_INT, i=sbx);
*/
    GI sbx = _AR(bx) - MAX_sBx;		// sBx

    _RA_T2(GT_INT, i=sbx);
}


//================================================================
/*!@brief
  OP_LOADSYM

  R(A) := Syms(Bx)
*/
__GURU__ void
uc_loadsym(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    U32 bx  = GET_Bx(code);
*/
	U32 bx  = _AR(bx);
    GS  sid = name2id(VM_SYM(vm, bx));

	_RA_T2(GT_SYM, i=sid);
}

//================================================================
/*!@brief
  OP_LOADNIL

  R(A) := nil
*/
__GURU__ void
uc_loadnil(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
    _RA_T2(GT_NIL, i=0);
}

//================================================================
/*!@brief
  OP_LOADSELF

  R(A) := self
*/
__GURU__ void
uc_loadself(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
    _RA2(*_R0);              		// [ra] <= class
}

//================================================================
/*!@brief
  OP_LOADT

  R(A) := true
*/
__GURU__ void
uc_loadt(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
    _RA_T2(GT_TRUE, i=0);
}

//================================================================
/*!@brief
  OP_LOADF

  R(A) := false
*/
__GURU__ void
uc_loadf(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
    _RA_T2(GT_FALSE, i=0);
}

//================================================================
/*!@brief
  OP_GETGLOBAL

  R(A) := getglobal(Syms(Bx))
*/
__GURU__ void
uc_getglobal(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    U32 bx  = GET_Bx(code);
*/
	U32 bx = _AR(bx);
    GS sid  = name2id(VM_SYM(vm, bx));

    guru_obj *obj = global_object_get(sid);

    _RA_X2(obj);
}

//================================================================
/*!@brief
  OP_SETGLOBAL

  setglobal(Syms(Bx), R(A))
*/
__GURU__ void
uc_setglobal(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    U32 bx  = GET_Bx(code);
*/
	U32 bx  = _AR(bx);
    GS  sid = name2id(VM_SYM(vm, bx));

    global_object_add(sid, _R(a));
}

//================================================================
/*!@brief
  OP_GETIV

  R(A) := ivget(Syms(Bx))
*/
__GURU__ void
uc_getiv(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 bx = GET_Bx(code);
*/
	U32 bx   = _AR(bx);
    U8P name = VM_SYM(vm, bx);
    GS sid   = name2id(name+1);					// skip '@'
    GV ret   = ostore_get(_R0, sid);

    _RA2(ret);
}

//================================================================
/*!@brief
  OP_SETIV

  ivset(Syms(Bx),R(A))
*/
__GURU__ void
uc_setiv(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 bx = GET_Bx(code);
*/
	U32 bx   = _AR(bx);
    U8P name = VM_SYM(vm, bx);
    GS  sid  = name2id(name+1);			// skip '@'

    ostore_set(_R0, sid, _R(a));
}

//================================================================
/*!@brief
  OP_GETCONST

  R(A) := constget(Syms(Bx))
*/
__GURU__ void
uc_getconst(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    U32 bx  = GET_Bx(code);
*/
	U32 bx  = _AR(bx);
    GS  sid = name2id(VM_SYM(vm, bx));

    guru_obj *obj = const_object_get(sid);

    _RA_X2(obj);
}

//================================================================
/*!@brief
  OP_SETCONST

  constset(Syms(Bx),R(A))
*/
__GURU__ void
uc_setconst(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    U32 bx  = GET_Bx(code);
*/
	U32 bx  = _AR(bx);
	GS  sid = name2id(VM_SYM(vm, bx));

    const_object_add(sid, _R(a));
}

//================================================================
/*!@brief
  OP_GETUPVAR

  R(A) := uvget(B,C)
*/
__GURU__ void
uc_getupvar(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
    U32 rc = GET_RC(code);   	// level up
*/
	U32 b = _AR(b);
	U32 c = _AR(c);

    guru_state *st = vm->state;

    U32 n = (c+1) << 1;			// depth of call stack (2 for each level)
    while (n > 0){				// walk up call stack
        st = st->prev;
        n--;
    }
    GV *uregs = st->regs;		// outer scope register file

    _RA2(uregs[b]);          	// ra <= up[rb]
}

//================================================================
/*!@brief
  OP_SETUPVAR

  uvset(B,C,R(A))
*/
__GURU__ void
uc_setupvar(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
    U32 rc = GET_RC(code);   				// UP level
*/
	U32 b = _AR(b);
	U32 c = _AR(c);

    guru_state *st = vm->state;

    U32 n = (c+1) << 1;						// 2 per outer scope level
    while (n > 0){
        st = st->prev;	CHECK_NULL(st);
        n--;
    }
    GV *uregs = st->regs;					// pointer to caller's register file

    ref_dec(&uregs[b]);
    uregs[b] = *_R(a);                   	// update outer-scope vars
}

//================================================================
/*!@brief
  OP_JMP

  pc += sBx
*/
__GURU__ void
uc_jmp(guru_vm *vm)
{
//	U32 code = vm->bytecode;
//  vm->state->pc += GET_sBx(code) - 1;
	U32 sbx = _AR(bx) - MAX_sBx;

	vm->state->pc += sbx -1;
}

//================================================================
/*!@brief
  OP_JMPIF

  if R(A) pc += sBx
*/
__GURU__ void
uc_jmpif (guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

    if (regs[GET_RA(code)].gt > GT_FALSE) {
        vm->state->pc += GET_sBx(code) - 1;
    }
*/
	U32 sbx = _AR(bx) - MAX_sBx;
	if (_R(a)->gt > GT_FALSE) {
		vm->state->pc += sbx - 1;
	}
}

//================================================================
/*!@brief
  OP_JMPNOT

  if not R(A) pc += sBx
*/
__GURU__ void
uc_jmpnot(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

	if (regs[GET_RA(code)].gt <= GT_FALSE) {
        vm->state->pc += GET_sBx(code) - 1;
    }
*/
	U32 sbx = _AR(bx) - MAX_sBx;
	if (_R(a)->gt <= GT_FALSE) {
		vm->state->pc += sbx - 1;
	}
}


//================================================================
/*!@brief
  OP_SEND / OP_SENDB

  OP_SEND   R(A) := call(R(A),Syms(B),R(A+1),...,R(A+C))
  OP_SENDB  R(A) := call(R(A),Syms(B),R(A+1),...,R(A+C),&R(A+C+1))
*/
__GURU__ void
_wipe_stack(GV *r, U32 rc)
{
    for (U32 i=0; i<rc; i++, r++) {		// sweep block parameters
    	ref_dec(r);
    	r->gt  = GT_EMPTY;				// clean up for stat dumper
    	r->cls = NULL;
    }
}

__GURU__ void
uc_send(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);  						// proc.sid
    U32 rc = GET_RC(code);  						// number of params
*/
	U32 b    = _AR(b);
	U32 c    = _AR(c);
    U8  *sym = VM_SYM(vm, b);						// get given symbol
	GS  sid  = name2id(sym); 						// get method sid of the current class
    GV  *obj = _R(a);								// call stack, obj is receiver object

	guru_proc *m = (guru_proc *)proc_by_sid(obj, sid);
    if (m==0) {										// eethod not found
    	_wipe_stack(obj+1, c+1);
    	SKIP(sym); 									// bail out
    }
    else if (HAS_IREP(m)) {							// a Ruby-based IREP
    	vm_state_push(vm, m->irep, obj, c);			// switch to callee's context
    }
    else if (m->func==prc_call) {					// C-based prc_call (hacked handler, it needs vm->state)
    	vm_proc_call(vm, obj, c);					// push into call stack, obj at stack[0]
    }
    else {											// other default C-based methods
    	m->func(obj, c);
    	_wipe_stack(obj+1, c+1);
    }
}

//================================================================
/*!@brief
  OP_CALL

  R(A) := self.call(frame.argc, frame.argv)
*/
__GURU__ void
uc_call(guru_vm *vm)
{
	guru_irep *irep = _R0->proc->irep;

	vm_state_push(vm, irep, _R0, 0);
}

//================================================================
/*!@brief
  OP_ENTER

  arg setup according to flags (23=5:5:1:5:5:1:1)
*/
__GURU__ void
uc_enter(guru_vm *vm)
{
/*
	U32 code  = vm->bytecode;
    U32 ax    = GET_Ax(code);
*/
	U32 ax   = _Ax;

    U32 arg0 = (ax >> 13) & 0x1f;  // default args
    U32 argc = (ax >> 18) & 0x1f;  // given args

    if (arg0 > 0){
        vm->state->pc += vm->state->argc - argc;
    }
}

//================================================================
/*!@brief
  OP_RETURN

  return R(A) (B=normal,in-block return/break)
*/
__GURU__ void
uc_return(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra   = GET_RA(code);
    GV  ret  = regs[ra];
    vm_state_pop(vm, ret, ra);		// pass return value
*/
	U32 a   = _AR(a);
	GV  ret = *_R(a);

    vm_state_pop(vm, ret, a);		// pass return value
}

//================================================================
/*!@brief
  OP_BLKPUSH (yield implementation)

  R(A) := block (16=6:1:5:4)
*/
__GURU__ void
uc_blkpush(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
    GV *prc = vm->state->regs+1;       		// get proc, regs[0] is the class

    assert(prc->gt==GT_PROC);				// ensure

    _RA2(*prc);             				// ra <= proc
}

//================================================================
/*!@brief
  OP_ADD

  R(A) := R(A)+R(A+1) (Syms[B]=:+,C=1)
*/
__GURU__ void
uc_add(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra  = GET_RA(code);
    GV  *r0 = &regs[ra];
    GV  *r1 = &regs[ra+1];
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    if (r0->gt==GT_INT) {
        if      (r1->gt==GT_INT) 	r0->i += r1->i;
#if GURU_USE_FLOAT
        else if (r1->gt==GT_FLOAT) {	// in case of Fixnum, Float
            r0->gt = GT_FLOAT;
            r0->f  = r0->i + r1->f;
        }
        else SKIP("Fixnum + ?");
    }
    else if (r0->gt==GT_FLOAT) {
        if      (r1->gt==GT_INT) 	r0->f += r1->i;
        else if (r1->gt==GT_FLOAT)	r0->f += r1->f;
        else SKIP("Float + ?");
#endif
    }
    else {    	// other case
    	uc_send(vm);			// should have already released regs[ra + n], ...
    }
    r1->gt = GT_EMPTY;
}

//================================================================
/*!@brief
  OP_ADDI

  R(A) := R(A)+C (Syms[B]=:+)
*/
__GURU__ void
uc_addi(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
	U32 ra = GET_RA(code);
	U32 rc = GET_RC(code);

    GV *r0 = &regs[ra];
*/
	GV *r0 = _R(a);
	U32 c  = _AR(c);

    if (r0->gt==GT_INT)     	r0->i += c;
#if GURU_USE_FLOAT
    else if (r0->gt==GT_FLOAT)	r0->f += c;
#else
    else QUIT("Float class");
#endif
}

//================================================================
/*!@brief
  OP_SUB

  R(A) := R(A)-R(A+1) (Syms[B]=:-,C=1)
*/
__GURU__ void
uc_sub(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);

    GV *r0 = &regs[ra];
    GV *r1 = &regs[ra+1];
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    if (r0->gt==GT_INT) {
        if      (r1->gt==GT_INT) 	r0->i -= r1->i;
#if GURU_USE_FLOAT
        else if (r1->gt==GT_FLOAT) {		// in case of Fixnum, Float
            r0->gt = GT_FLOAT;
            r0->f  = r0->i - r1->f;
        }
        else SKIP("Fixnum - ?");
    }
    else if (r0->gt==GT_FLOAT) {
        if      (r1->gt==GT_INT)	r0->f -= r1->i;
        else if (r1->gt==GT_FLOAT)	r0->f -= r1->f;
        else SKIP("Float - ?");
#endif
    }
    else {  // other case
    	uc_send(vm);
    }
    ref_clr(r1);
}

//================================================================
/*!@brief
  OP_SUBI

  R(A) := R(A)-C (Syms[B]=:-)
*/
__GURU__ void
uc_subi(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rc = GET_RC(code);

    GV *r0 = &regs[ra];
*/
	GV *r0 = _R(a);
	U32 c  = _AR(c);

    if (r0->gt==GT_INT) 		r0->i -= c;
#if GURU_USE_FLOAT
    else if (r0->gt==GT_FLOAT) 	r0->f -= c;
#else
    else QUIT("Float class");
#endif
}

//================================================================
/*!@brief
  OP_MUL

  R(A) := R(A)*R(A+1) (Syms[B]=:*)
*/
__GURU__ void
uc_mul(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    GV *r0 = &regs[ra];
    GV *r1 = &regs[ra+1];
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    if (r0->gt==GT_INT) {
        if      (r1->gt==GT_INT) 	r0->i *= r1->i;
#if GURU_USE_FLOAT
        else if (r1->gt==GT_FLOAT) {	// in case of Fixnum, Float
            r0->gt = GT_FLOAT;
            r0->f  = r0->i * r1->f;
        }
        else SKIP("Fixnum * ?");
    }
    else if (r0->gt==GT_FLOAT) {
        if      (r1->gt==GT_INT) 	r0->f *= r1->i;
        else if (r1->gt==GT_FLOAT)  r0->f *= r1->f;
        else SKIP("Float * ?");
#endif
    }
    else {   // other case
    	uc_send(vm);
    }
    ref_clr(r1);
}

//================================================================
/*!@brief
  OP_DIV

  R(A) := R(A)/R(A+1) (Syms[B]=:/)
*/
__GURU__ void
uc_div(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    GV *r0 = &regs[ra];
    GV *r1 = &regs[ra+1];
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    if (r0->gt==GT_INT) {
        if      (r1->gt==GT_INT) 	r0->i /= r1->i;
#if GURU_USE_FLOAT
        else if (r1->gt==GT_FLOAT) {		// in case of Fixnum, Float
            r0->gt = GT_FLOAT;
            r0->f  = r0->i / r1->f;
        }
        else SKIP("Fixnum / ?");
    }
    else if (r0->gt==GT_FLOAT) {
        if      (r1->gt==GT_INT) 	r0->f /= r1->i;
        else if (r1->gt==GT_FLOAT)	r0->f /= r1->f;
        else SKIP("Float / ?");
#endif
    }
    else {   // other case
    	uc_send(vm);
    }
    ref_clr(r1);
}

//================================================================
/*!@brief
  OP_EQ

  R(A) := R(A)==R(A+1)  (Syms[B]=:==,C=1)
*/
__GURU__ void
uc_eq(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    GT  tt = GT_BOOL(guru_cmp(&regs[ra], &regs[ra+1])==0);

    regs[ra+1].gt = GT_EMPTY;
    _RA_T(tt, i=0);
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    GT tt = GT_BOOL(guru_cmp(r0, r1)==0);
    r1->gt = GT_EMPTY;

    _RA_T2(tt, i=0);
}

// macro for comparators
#define ncmp(r0, op, r1)								\
do {													\
	if ((r0)->gt==GT_INT) {								\
		if ((r1)->gt==GT_INT) {							\
			(r0)->gt = GT_BOOL((r0)->i op (r1)->i);		\
		}												\
		else if ((r1)->gt==GT_FLOAT) {					\
			(r0)->gt = GT_BOOL((r0)->i op (r1)->f);		\
		}												\
	}													\
	else if ((r0)->gt==GT_FLOAT) {						\
		if ((r1)->gt==GT_INT) {							\
			(r0)->gt = GT_BOOL((r0)->f op (r1)->i);		\
		}												\
		else if ((r1)->gt==GT_FLOAT) {					\
			(r0)->gt = GT_BOOL((r0)->f op (r1)->f);		\
		}												\
	}													\
	else {												\
		uc_send(vm);						\
	}													\
    ref_clr(r1);										\
} while (0)

//================================================================
/*!@brief
  OP_LT

  R(A) := R(A)<R(A+1)  (Syms[B]=:<,C=1)
*/
__GURU__ void
uc_lt(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
	U32 ra = GET_RA(code);
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

	ncmp(r0, <, r1);

    r1->gt = GT_EMPTY;
}

//================================================================
/*!@brief
  OP_LE

  R(A) := R(A)<=R(A+1)  (Syms[B]=:<=,C=1)
*/
__GURU__ void
uc_le(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    ncmp(r0, <=, r1);
    r1->gt = GT_EMPTY;
}

//================================================================
/*!@brief
  OP_GT

  R(A) := R(A)>=R(A+1) (Syms[B]=:>=,C=1)
*/
__GURU__ void
uc_gt(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    ncmp(r0, >, r1);

    r1->gt = GT_EMPTY;
}

//================================================================
/*!@brief
  OP_GE

  R(A) := R(A)>=R(A+1) (Syms[B]=:>=,C=1)
*/
__GURU__ void
uc_ge(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
*/
	GV *r0 = _R(a);
	GV *r1 = r0+1;

    ncmp(r0, >=, r1);
    r1->gt = GT_EMPTY;
}

//================================================================
/*!@brief
  Create string object

  R(A) := str_dup(Lit(Bx))
*/
__GURU__ void
uc_string(guru_vm *vm)
{
#if GURU_USE_STRING
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

	U32 ra = GET_RA(code);
    U32 bx = GET_Bx(code);
*/
	U32 bx   = _AR(bx);
    U8  *str = (U8 *)VM_VAR(vm, bx);			// string pool var
    GV  ret  = guru_str_new(str);				// rc set to 1 already

    _RA2(ret);
#else
    QUIT("String class");
#endif
}

//================================================================
/*!@brief
  String Catination

  str_cat(R(A),R(B))
*/
__GURU__ void
uc_strcat(guru_vm *vm)
{
#if GURU_USE_STRING
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);

    GV *va  = &regs[ra];
    GV *vb  = &regs[rb];
 */
	GV *va = _R(a);
	GV *vb = _R(b);
    GS sid  = name2id((U8P)"to_s");				// from global symbol pool

    guru_proc *pa = proc_by_sid(va, sid);
    guru_proc *pb = proc_by_sid(vb, sid);

    if (pa) pa->func(va, 0);					// can it be an IREP?
    if (pb) pb->func(vb, 0);

    GV ret = guru_str_add(va, vb);				// ref counts updated

    ref_dec(va);								// free both strings
    ref_dec(vb);
    vb->gt = GT_EMPTY;

    _RA_X2(&ret);

#else
    QUIT("String class");
#endif
}

//================================================================
/*!@brief
  Create Array object

  R(A) := ary_new(R(B),R(B+1)..R(B+C))
*/
__GURU__ void
uc_array(guru_vm *vm)
{
#if GURU_USE_ARRAY
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

    U32 ra  = GET_RA(code);
    U32 rb  = GET_RB(code);
    U32 n   = GET_RC(code);

    GV  ret = (GV)guru_array_new(n);	// ref_cnt is 1 already
    guru_array *h  = ret.array;

    GV *s = &regs[rb];					// source elements
*/
    U32 n   = _AR(c);
    GV  ret = (GV)guru_array_new(n);	// ref_cnt is 1 already
    guru_array *h  = ret.array;

    GV *s = _R(b);						// source elements
	GV *d = h->data;					// target
	for (U32 i=0; i<(n); i++, ref_inc(s), *d++=*s, s->gt=GT_EMPTY, s++);
    h->n = n;

    _RA2(ret);							// no need to ref_inc
#else
    QUIT("Array class");
#endif
}

//================================================================
/*!@brief
  Create Hash object

  R(A) := hash_new(R(B),R(B+1)..R(B+C))
*/
__GURU__ void
uc_hash(guru_vm *vm)
{
#if GURU_USE_ARRAY
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
    U32 n  = GET_RC(code);							// entries of hash
	U32 sz = sizeof(GV) * (n<<1);						// size of k,v pairs

    GV *p  = &regs[rb];
    GV ret = guru_hash_new(n);							// ref_cnt is already set to 1
    guru_hash  *h = ret.hash;

    MEMCPY((U8P)h->data, (U8P)p, sz);					// copy k,v pairs
*/
	U32 n  = _AR(c);
    GV *p  = _R(b);
    GV ret = guru_hash_new(n);							// ref_cnt is already set to 1
    guru_hash  *h = ret.hash;

    for (U32 i=0; i<(h->n=(n<<1)); i++, p++) {
    	p->gt = GT_EMPTY;								// clean up call stack
    }
    _RA2(ret);						          			// set return value on stack top
#else
    QUIT("Hash class");
#endif
}

//================================================================
/*!@brief
  OP_RANGE

  R(A) := range_new(R(B),R(B+1),C)
*/
__GURU__ void
uc_range(guru_vm *vm)
{
#if GURU_USE_ARRAY
/*
 	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
    U32 n  = GET_RC(code);

    GV *pb = &regs[rb];
*/
	U32 n   = _AR(c);
	GV  *p0 = _R(b);
	GV  *p1 = p0+1;

    GV ret = guru_range_new(p0, p1, n);		// pb, pb+1 ref cnt will be increased
    p1->gt = GT_EMPTY;

    _RA_X2(&ret);							// release and  reassign
#else
    QUIT("Range class");
#endif
}

//================================================================
/*!@brief
  OP_LAMBDA

  R(A) := lambda(SEQ[Bz],Cz)
*/
__GURU__ void
uc_lambda(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;
	int ra = GET_RA(code);
    int rb = GET_b(code);      			// sequence position in irep list
*/
	U32 b = _AR(bx) >> 2;

    guru_proc *prc = (guru_proc *)guru_alloc(sizeof(guru_proc));

    prc->sid  = 0xffffffff;				// anonymous function
    prc->func = NULL;
    prc->irep = VM_REPS(vm, b);			// fetch from children irep list

    _RA_T2(GT_PROC, proc=prc);			// regs[ra].proc = prc
}

//================================================================
/*!@brief
  OP_CLASS

  R(A) := newclass(R(A),Syms(B),R(A+1))
  Syms(B): class name
  R(A+1): super class
*/
__GURU__ void
uc_class(guru_vm *vm)
{
/*
	GV *regs = vm->state->regs;
	U32 code = vm->bytecode;

    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
*/
	U32 b = _AR(b);
	GV *ra1 = _R(a)+1;

    guru_class *super = (ra1->gt==GT_CLASS) ? ra1->cls : guru_class_object;
    const U8P  name   = VM_SYM(vm, b);
    guru_class *cls   = guru_define_class(name, super);

    _RA_T2(GT_CLASS, cls=cls);
}

//================================================================
/*!@brief
  OP_EXEC

  R(A) := blockexec(R(A),SEQ[Bx])
*/
__GURU__ void
uc_exec(guru_vm *vm)
{
/*
	U32 code  = vm->bytecode;
	GV  *regs = vm->state->regs;

    U32 ra = GET_RA(code);						// return register
    U32 bx = GET_Bx(code);						// irep index
*/
	U32 bx = _AR(bx);

    guru_irep *irep = VM_REPS(vm, bx);			// child IREP[rb]

    vm_state_push(vm, irep, _R(a), 0);			// push call stack
}

//================================================================
/*!@brief
  OP_METHOD

  R(A).newmethod(Syms(B),R(A+1))
*/
__GURU__ void
uc_method(guru_vm *vm)
{
/*
	GV  *regs = vm->state->regs;
	U32 code  = vm->bytecode;
    U32 ra = GET_RA(code);
    U32 rb = GET_RB(code);
    GV  *obj  = regs+ra;					// receiver object
*/
	U32 a    = _AR(a);
	U32 b    = _AR(b);
	GV  *obj = _R(a);

    assert(obj->gt == GT_CLASS);			// enforce class checking

    // check whether the name has been defined in current class (i.e. vm->state->klass)
    GS	      sid  = name2id(VM_SYM(vm, b));	// lookup symbol id (which should be registered already)
    guru_proc *prc = proc_by_sid(obj, sid);		// fetch proc from obj->klass->vtbl

    if (prc != NULL) {
    	// same proc name exists (in either current or parent class)
    	// do nothing for now
    }
    prc = (obj+1)->proc;					// use the proc specified by OP_LAMBDA

    _LOCK;

    // add proc to class
    guru_class 	*cls = obj->cls;
    prc->sid  = sid;						// assign sid to proc, overload if prc already exists
    prc->next = cls->vtbl;					// add to top of vtable
    cls->vtbl = prc;

    _UNLOCK;

    _wipe_stack(vm->state->regs+1, a+1);
}

//================================================================
/*!@brief
  OP_TCLASS

  R(A) := target_class
*/
__GURU__ void
uc_tclass(guru_vm *vm)
{
/*
	U32 code  = vm->bytecode;
	GV  *regs = vm->state->regs;
	U32 ra = GET_RA(code);
*/
	_RA_T2(GT_CLASS, cls=vm->state->klass);
}

//================================================================
/*!@brief
  OP_STOP and OP_ABORT

  stop VM (OP_STOP)
  stop VM without release memory (OP_HOLD)
*/
__GURU__ void
uc_stop(guru_vm *vm)
{
	vm->run  = VM_STATUS_STOP;	// VM suspended
}

//===========================================================================================
// GURU engine
//===========================================================================================
//================================================================
/*!@brief
  GURU Instruction Unit - Prefetcher (fetch bytecode and decode)

  @param  vm    A pointer of VM.
  @retval 0  No error.
*/
__GURU__ void
ucode_prefetch(guru_vm *vm)
{
	U32 b = vm->bytecode = VM_BYTECODE(vm);	// fetch from vm->state->pc
	U32 n = b >> 7;	      					// operands
//	vm->opn = n;							// keep operands
//	vm->op  = b & 0x7f;      				// opcode (cannot take address from bitfield yet)
	vm->ar  = *((GAR *)&n);        			// operands struct/union

	vm->state->pc++;				// advance program counter (ready for next fetch)
}

__GURU__ void
ucode_exec(guru_vm *vm)
{
	//=======================================================================================
	// GURU dispatcher unit
	//=======================================================================================
    switch (vm->op) {
    // LOAD,STORE
    case OP_LOADL:      uc_loadl     (vm); break;
    case OP_LOADI:      uc_loadi     (vm); break;
    case OP_LOADSYM:    uc_loadsym   (vm); break;
    case OP_LOADNIL:    uc_loadnil   (vm); break;
    case OP_LOADSELF:   uc_loadself  (vm); break;
    case OP_LOADT:      uc_loadt     (vm); break;
    case OP_LOADF:      uc_loadf     (vm); break;
    // VARIABLES
    case OP_GETGLOBAL:  uc_getglobal (vm); break;
    case OP_SETGLOBAL:  uc_setglobal (vm); break;
    case OP_GETIV:      uc_getiv     (vm); break;
    case OP_SETIV:      uc_setiv     (vm); break;
    case OP_GETCONST:   uc_getconst  (vm); break;
    case OP_SETCONST:   uc_setconst  (vm); break;
    case OP_GETUPVAR:   uc_getupvar  (vm); break;
    case OP_SETUPVAR:   uc_setupvar  (vm); break;
    // BRANCH
    case OP_JMP:        uc_jmp       (vm); break;
    case OP_JMPIF:      uc_jmpif     (vm); break;
    case OP_JMPNOT:     uc_jmpnot    (vm); break;
    // CALL
    case OP_SEND:
    case OP_SENDB:      uc_send      (vm); break;
    case OP_CALL:       uc_call      (vm); break;
    case OP_ENTER:      uc_enter     (vm); break;
    case OP_RETURN:     uc_return    (vm); break;
    case OP_BLKPUSH:    uc_blkpush   (vm); break;
    // ALU
    case OP_MOVE:       uc_move      (vm); break;
    case OP_ADD:        uc_add       (vm); break;
    case OP_ADDI:       uc_addi      (vm); break;
    case OP_SUB:        uc_sub       (vm); break;
    case OP_SUBI:       uc_subi      (vm); break;
    case OP_MUL:        uc_mul       (vm); break;
    case OP_DIV:        uc_div       (vm); break;
    case OP_EQ:         uc_eq        (vm); break;
    case OP_LT:         uc_lt        (vm); break;
    case OP_LE:         uc_le        (vm); break;
    case OP_GT:         uc_gt        (vm); break;
    case OP_GE:         uc_ge        (vm); break;
    // BUILT-IN class (TODO: tensor)
#if GURU_USE_STRING
    case OP_STRING:     uc_string    (vm); break;
    case OP_STRCAT:     uc_strcat    (vm); break;
#endif
#if GURU_USE_ARRAY
    case OP_ARRAY:      uc_array     (vm); break;
    case OP_HASH:       uc_hash      (vm); break;
    case OP_RANGE:      uc_range     (vm); break;
#endif
    // CLASS, PROC (STACK ops)
    case OP_LAMBDA:     uc_lambda    (vm); break;
    case OP_CLASS:      uc_class     (vm); break;
    case OP_EXEC:       uc_exec      (vm); break;
    case OP_METHOD:     uc_method    (vm); break;
    case OP_TCLASS:     uc_tclass    (vm); break;
    // CONTROL
    case OP_STOP:       uc_stop      (vm); break;
    case OP_NOP:        uc_nop       (vm); break;
    default:
    	PRINTF("?OP=0x%04x\n", vm->op);
    	vm->err = 1;
    	break;
    }
}

