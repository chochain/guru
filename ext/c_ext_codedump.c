...
static int
print_r(mrbc_irep *irep, size_t n, int pre)
{
    size_t i;
    ...
    for (i=0; i+1<irep->nlocals; i++) {
        if (irep->lv[i].r == n) {
            mrbc_sym sym = irep->lv[i].name;
            if (pre) printf(" ");
            printf("R%d:%s", (int)n, symid2name(sym));
            return 1;
        }
    }
    return 0;
...
static void
print_lv(mrbc_irep *irep, mrbc_code c, int r)
{
    int pre = 0;
    ...
    printf("\t; ");
    if (r & RA) {
        pre = print_r(irep, GETARG_A(c), 0);
    }
    if (r & RB) {
        print_r(irep, GETARG_B(c), pre);
    }
    printf("\n");
}

static void
codedump(mrbc_irep *irep)
{
#ifndef MRBC_DISABLE_STDIO
    int i;
    int ai;
    mrbc_code c;
    ...
    for (i = 0; i < (int)irep->ilen; i++) {
        ai = mrbc_gc_arena_save(mrb);

        next_file = mrbc_debug_get_filename(irep, i);
        if (next_file && file != next_file) {
            printf("file: %s\n", next_file);
            file = next_file;
        }
        line = mrbc_debug_get_line(irep, i);
        if (line < 0) {
            printf("      ");
        }
        else {
        ...
        case OP_LOADL:
        {
            mrbc_value v = irep->pool[GETARG_Bx(c)];
            mrbc_value s = mrbc_inspect(v);
        ...
        case OP_LOADSYM:
            printf("OP_LOADSYM\tR%d\t:%s", GETARG_A(c),
                   symid2name(irep->syms[GETARG_Bx(c)]));
            print_lv(irep, c, RA);
            break;
        case OP_LOADNIL:
        ...
        case OP_SEND:
            printf("OP_SEND\tR%d\t:%s\t%d\n", GETARG_A(c),
                   symid2name(irep->syms[GETARG_B(c)]),
                   GETARG_C(c));
            break;
        case OP_SENDB:
        ...
        {
            mrbc_value v = irep->pool[GETARG_Bx(c)];
            mrbc_value s = mrbc_str_dump(mrbc_str_new(RSTRING_PTR(v), RSTRING_LEN(v)));
...
void
mrbc_codedump_all(struct RProc *proc)
{
